...
const Skills = () => {
  ...
  return (
    <div className="w-full flex flex-col gap-8 sm:gap-12"> 
      ...
      <div className="skills-section grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {skills &&
          skills.map((element) => {
            return (
              <Card className="h-fit p-7 flex flex-col justify-center items-center gap-3" key={element._id}>
                <img
                  ...
                  alt="skill"
                  className="s-img h-12 sm:h-24 w-auto"
                />
                <p className="s-img text-muted-foreground text-center">
                  {element.title}
                </p>
              </Card>
            );
