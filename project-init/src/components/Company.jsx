...
const Company = ({ companyId, name, subtitle, userPicturePath }) => {
  ...
  const isCompany = companies.find((company) => company._id === companyId);

  const patchCompany = async () => {
    const response = await fetch(
      `http://localhost:4001/api/v1/user/${_id}/${companyId}`,
      {
        method: "PATCH",
        headers: {
